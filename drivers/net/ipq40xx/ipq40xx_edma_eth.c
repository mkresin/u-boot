...
static int ipq40xx_eth_init(struct eth_device *eth_dev, bd_t *this)
{
	...
	char *dp[] = {"Half", "Full"};
	/* patch for AR8035 */
	u32 p5_status_reg_cur_val;
	u32 p5_status_reg_new_val;
	hw = &c_info->hw;
	/*
...
/* patch for AR8035 -- start */
/*
	get phy 0 status will update mac 1
	get phy 1 status will update mac 2
	get phy 2 status will update mac 3
	get phy 3 status will update mac 4
	get phy 4 status will update mac 5
	so we need to only get phy 4 status and update mac 5
*/
	status = phy_get_ops->phy_get_link_status(priv->mac_unit, 1);
	if (status == 1){
		// no link
		linkup = -1;
		printf("No link !\n");
	...
	phy_get_ops->phy_get_speed(priv->mac_unit, 1, &speed);
	phy_get_ops->phy_get_duplex(priv->mac_unit, 1, &duplex);

	printf ("eth%d PHY%d %s Speed :%d %s duplex\n",
		priv->mac_unit, 1, lstatus[status], speed,	dp[duplex]);

	p5_status_reg_cur_val = readl((void __iomem *)(0xC000000 + 0x0090));
	switch(speed){
		case FAL_SPEED_10:
			if(duplex == 0){
				// half
				p5_status_reg_new_val = 0xfc;
			}else{
				// full
				p5_status_reg_new_val = 0x7c;
			}
			break;
		case FAL_SPEED_100:
			if(duplex == 0){
				// half
				p5_status_reg_new_val = 0xfd;
			}else{
				// full
				p5_status_reg_new_val = 0x7d;
			}
			break;
		case FAL_SPEED_1000:
			if(duplex == 0){
				// half
				p5_status_reg_new_val = 0xfe;
			}else{
				// full
				p5_status_reg_new_val = 0x7e;
			}
			break;
		default:
	...
	writel(p5_status_reg_new_val, ((void __iomem *)(0xC000000 + 0x0090)));
/* patch for AR8035 -- end */
