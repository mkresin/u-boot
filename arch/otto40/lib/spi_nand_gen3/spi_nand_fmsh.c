...
__SECTION_INIT_PHASE void 
FM25S01_disable_on_die_ecc(u32_t cs)
{
...
probe_fmsh_spi_nand_chip(void)
{
    ...
    for(i=0 ; i<ELEMENT_OF_SNAF_INFO(fmsh_chip_info) ; i++){
        ...
        {  
            ...
            if(fmsh_chip_info[i].dev_id == DID_FM25S01){
                FM25S01_disable_on_die_ecc(0);
                #if (__DEVICE_REASSIGN&__DEVICE_USING_QIO)
                    fmsh_chip_info[i]._cmd_info = _nsu_x2_cmd_info_ptr;
                #endif
            }else if(fmsh_chip_info[i].dev_id == DID_FM25G02C){
