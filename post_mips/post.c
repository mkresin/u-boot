...
extern unsigned int is_ctrlc(void);
...
void post_bootmode_init(void)
{
	...
	if (post_hotkeys_pressed() && !(bootmode & POST_POWERTEST))
		...
		newword = BOOTMODE_MAGIC | POST_POWERON | POST_NORMAL2;
	else if (bootmode == POST_POWERON || bootmode == POST_SLOWTEST)
		newword = BOOTMODE_MAGIC | POST_NORMAL2;
	else
		/* Use old value */
...
{
	printf("post_run_single flags = 0x%x, name=%s,test_flags = 0x%x postalways = 0x%x,postmem=0x%x\n",flags,test->name,test_flags,POST_ALWAYS,POST_MEM);
	...
	if (name == NULL) {
		...
		if (post_bootmode_get(&last) & POST_POWERTEST) {
			...
			if (last < post_list_size &&
				...
				//printf("post_run start %d %x \n",last,test_flags[last]);
