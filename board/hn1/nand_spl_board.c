...
void nand_spl_init(void)
{
  ...
  asm("sync");
  nand_read_page((CONFIG_NAND_SPL_BLOCK_SIZE/CONFIG_NAND_PAGE_SIZE)-1,page_buf);
  asm("sync");
  ...
  if(buffer[0]==ddr_magic)
   ...
   asc_puts("\nDDR controller active, start tune_ddr. \n");	 
