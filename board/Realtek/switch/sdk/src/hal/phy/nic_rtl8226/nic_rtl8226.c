    ...
    )
{
    ...
    UINT16 phydata3 = 0;
    UINT16 _6A21_5 = 0;
    UINT16 adccal_offset_p0 = 0;
    UINT16 adccal_offset_p1 = 0;
    UINT16 adccal_offset_p2 = 0;
    UINT16 adccal_offset_p3 = 0;
    UINT16 rg_lpf_cap_xg_p0 = 0;
    UINT16 rg_lpf_cap_xg_p1 = 0;
    UINT16 rg_lpf_cap_xg_p2 = 0;
    UINT16 rg_lpf_cap_xg_p3 = 0;
    UINT16 rg_lpf_cap_p0 = 0;
    UINT16 rg_lpf_cap_p1 = 0;
    UINT16 rg_lpf_cap_p2 = 0;
    UINT16 rg_lpf_cap_p3 = 0;


    status = MmdPhyRead(hDevice, MMD_VEND1, 0x6A21, &_6A21_5);
    if (status != SUCCESS)
        goto exit;
    ...
    Flag_mdi_swap = (phydata & 0x0002)? 1:0;
    if (!Flag_mdi_swap){
	phy_osal_printf("MDI already swap \n");
	goto exit;
	}

    phydata0 = phydata & 0xFFE0;
    phydata0 |= 0x0001;

    status = MmdPhyWrite(hDevice, MMD_VEND2, 0xD068, phydata0);
    if (status != SUCCESS)
        goto exit;

    status = MmdPhyRead(hDevice, MMD_VEND2, 0xD06A, &adccal_offset_p0);
    if (status != SUCCESS)
        goto exit;

    phydata1 = phydata & 0xFFE0;
    phydata1 |= 0x0009;

    status = MmdPhyWrite(hDevice, MMD_VEND2, 0xD068, phydata1);
    if (status != SUCCESS)
        goto exit;

    status = MmdPhyRead(hDevice, MMD_VEND2, 0xD06A, &adccal_offset_p1);
    if (status != SUCCESS)
        goto exit;

    phydata2 = phydata & 0xFFE0;
    phydata2 |= 0x0011;

    status = MmdPhyWrite(hDevice, MMD_VEND2, 0xD068, phydata2);
    if (status != SUCCESS)
        goto exit;

    status = MmdPhyRead(hDevice, MMD_VEND2, 0xD06A, &adccal_offset_p2);
    if (status != SUCCESS)
        goto exit;

    phydata3 = phydata & 0xFFE0;
    phydata3 |= 0x0019;

    status = MmdPhyWrite(hDevice, MMD_VEND2, 0xD068, phydata3);
    if (status != SUCCESS)
        goto exit;

    status = MmdPhyRead(hDevice, MMD_VEND2, 0xD06A, &adccal_offset_p3);
    if (status != SUCCESS)
        goto exit;

    status = MmdPhyRead(hDevice, MMD_VEND2, 0xBD5A, &phydata);
    if (status != SUCCESS)
        goto exit;
    
    rg_lpf_cap_xg_p0 = phydata & 0x001F;
    rg_lpf_cap_xg_p1 = phydata & 0x1F00;

    status = MmdPhyRead(hDevice, MMD_VEND2, 0xBD5C, &phydata);
    if (status != SUCCESS)
        goto exit;

    rg_lpf_cap_xg_p2 = phydata & 0x001F;
    rg_lpf_cap_xg_p3 = phydata & 0x1F00;

    status = MmdPhyRead(hDevice, MMD_VEND2, 0xBC18, &phydata);
    if (status != SUCCESS)
        goto exit;

    rg_lpf_cap_p0 = phydata & 0x001F;
    rg_lpf_cap_p1 = phydata & 0x1F00;

    status = MmdPhyRead(hDevice, MMD_VEND2, 0xBC1A, &phydata);
    if (status != SUCCESS)
        goto exit;

    rg_lpf_cap_p2 = phydata & 0x001F;
    rg_lpf_cap_p3 = phydata & 0x1F00;

    if (Enable)
    {
        _6A21_5 |= (BIT_5);
        status = MmdPhyWrite(hDevice, MMD_VEND1, 0x6A21, _6A21_5);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xD06A, adccal_offset_p3);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xD06A, adccal_offset_p2);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xD06A, adccal_offset_p1);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xD06A, adccal_offset_p0);
        if (status != SUCCESS)
            goto exit;
        ...
        phydata &= 0xE0E0;
        phydata = ( rg_lpf_cap_xg_p3 >>8 ) | (rg_lpf_cap_xg_p2 << 8) | phydata;
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xBD5A, phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        phydata &= 0xE0E0;
        phydata = (rg_lpf_cap_xg_p1 >>8) | (rg_lpf_cap_xg_p0 <<8)| phydata;
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xBD5C, phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        phydata &= 0xE0E0;
        phydata = (rg_lpf_cap_p3 >>8) | (rg_lpf_cap_p2 <<8) | phydata;
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xBC18, phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        phydata &= 0xE0E0;
        phydata = (rg_lpf_cap_p1 >>8) | (rg_lpf_cap_p0 <<8) | phydata;
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xBC1A, phydata);
        if (status != SUCCESS)
            goto exit;
...
/*    else
    {
        ...
        if (status != SUCCESS)
            goto exit;
        phydata &= 0xE0E0;
        phydata = rg_lpf_cap_xg_p1 | rg_lpf_cap_xg_p0 | phydata;
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xBD5A, phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        if (status != SUCCESS)
            goto exit;
        phydata &= 0xE0E0;
        phydata = rg_lpf_cap_xg_p2 | rg_lpf_cap_xg_p3 | phydata;
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xBD5C, phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        if (status != SUCCESS)
            goto exit;
        phydata &= 0xE0E0;
        phydata = rg_lpf_cap_p1 | rg_lpf_cap_p0 | phydata;
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xBC18, phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        if (status != SUCCESS)
            goto exit;
        phydata &= 0xE0E0;
        phydata = rg_lpf_cap_p3 | rg_lpf_cap_p2 | phydata;
        status = MmdPhyWrite(hDevice, MMD_VEND2, 0xBC1A, phydata);
        if (status != SUCCESS)
            goto exit;
    ...
    )
{
    ...
    if (linkOK)
    {
        ...
        if (Speed == 10)
        {
            phy_osal_printf("RTCT is not supported when port link at 10M.\n");
            status = FAILURE;
            goto exit;
        }
    }
