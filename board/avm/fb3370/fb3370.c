...
#define GPIO_LED_POWER_GREEN	32
#define GPIO_LED_POWER_RED	33
#define GPIO_POWER_OFF		45
#define GPIO_PHY0_RESET		37
#define GPIO_PHY1_RESET		44

static void gpio_init(void)
{
	/* set power off gpio to output to prevent freeze after 30sec */
	gpio_direction_output(GPIO_POWER_OFF, 0);
...
void show_boot_progress(int arg)
{
	if (arg >= 0) {
		...
		gpio_set_value(GPIO_LED_POWER_RED, 1);
		gpio_set_value(GPIO_LED_POWER_GREEN, 0);
	} else {
		/* Failure - turn off green power LED and turn on red power LED */
		gpio_set_value(GPIO_LED_POWER_GREEN, 1);
		gpio_set_value(GPIO_LED_POWER_RED, 0);
	}
}
#endif
...
int board_eth_init(bd_t * bis)
{
	...
	/* release external phy reset */
	gpio_direction_output(GPIO_PHY0_RESET, 1);
	gpio_direction_output(GPIO_PHY1_RESET, 1);
