...
static void leds_default(void)
{
    ...
    *(AR10_GPIO_P1_OD) &= ~(1 << 1);
     *AR10_GPIO_P1_OUT |=  (1 << 1); /* By default switch off */

    /* green power LED - gpio 6: port 0, pin 6 */
    *(AR10_GPIO_P0_DIR) |= (1 << 6);
    *(AR10_GPIO_P0_ALTSEL0) &= ~(1 << 6);
    *(AR10_GPIO_P0_ALTSEL1) &= ~(1 << 6);
    *(AR10_GPIO_P0_OD) &= ~(1 << 6);
     *AR10_GPIO_P0_OUT &= ~(1 << 6); /* By default switch on */
#endif
}
...
void nand_spl_init(void)
{
    ...
    for(i=0;i<6;i++){
    buffer[i] = *(volatile u32*)(page_buf+CONFIG_NAND_PAGE_SIZE-24+i*4); 
    }
